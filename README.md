...
For calibration: scored with this rubric, **Airbnb's 2008 seed deck comes out at 7.1** (7.8 once adjusted for its stage) — a number you can reproduce from a fresh clone by running `python3 tools/score.py`, which re-derives it from the filled scorecard. Airbnb raised successfully at that score. It is the one anchored benchmark here, on purpose: the file in `input/` for Coinbase is a later marketing reconstruction, not the 2012 deck the company raised on, so this workspace deliberately publishes *no* Coinbase benchmark rather than an uncheckable one (`reference/scoring-rubric.md` §3 explains the retired 5.5). A 6 is not a failure — it's four named sections and a map to them.
